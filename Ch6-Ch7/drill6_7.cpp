...
constexpr char quit_token = 'x';
constexpr char print = '=';
constexpr char name = 'a';
constexpr char let = 'L';
constexpr char sqrt_token = 's';
constexpr char pow_token = 'p';
constexpr char result = ':';
const string declkey = "#"; //constexpr string since C++20 only
const string declkey_sqrt = "sqrt";
const string declkey_pow = "pow";
const string declkey_quit = "exit";
	...
	switch (ch)
	{
    	...
    	default: 
    	{
    		if (isalpha(ch))
    		{
    			...
    			if (s == declkey_sqrt) return Token{sqrt_token};
    			if (s == declkey_pow) return Token{pow_token};
    			if (s == declkey_quit) return Token{quit_token};
...
try {
	...
	cout << "You can use the following operators: '/','*','-','+','(',')','%','#','sqrt()','pow(,)'." <<"\n";
	cout << "To print the result use: '='." <<"\n";
	cout << "To exit the program use: 'exit'." <<"\n";
	...
	switch (t.kind)
	{
		...
		case sqrt_token:
		{
			t = ts.get();
  			if (t.kind != '(') error("'(' expected");
  			double d = expression();
  			if (d < 0) error("cannot calculate sqrt of negative numbers");
